...
const PHOTO_ZONES = [
  ...
  },
  {
    ...
    checks:["Dog on back with belly facing up","Full belly area from chest to groin visible","Flash enabled"],
    svgGuide:(<svg viewBox="0 0 160 100" width="100%" style={{borderRadius:"8px",display:"block"}}>
      <rect width="160" height="100" fill="#e8f0ee" rx="8"/>
...
// ── Deduplicate recommendation bullets ───────────────────────────────────────
function deduplicateBullets(results) {
  const allBullets = results
    .flatMap(r => (r.recommendations || "").split("|").map(b => b.trim()).filter(Boolean));
  ...
  for (const bullet of allBullets) {
    const isClosing = bullet.toLowerCase().includes("nobLfoods.com") ||
                      bullet.toLowerCase().includes("customerservice") ||
                      bullet.toLowerCase().includes("reach out");

    if (isClosing) { if (closing.length === 0) closing.push(bullet); continue; }

    // Normalize for dedup comparison
    const normalized = bullet.toLowerCase().replace(/[^a-z0-9 ]/g, "").replace(/\s+/g, " ").trim();
    // Check if any existing bullet shares 60%+ of key words
    const newWords = new Set(normalized.split(" ").filter(w => w.length > 4));
    let isDuplicate = false;
    for (const s of seen) {
      const sWords = new Set(s.split(" ").filter(w => w.length > 4));
      const overlap = [...newWords].filter(w => sWords.has(w)).length;
      if (overlap >= Math.min(newWords.size, sWords.size) * 0.6 && overlap >= 2) {
        isDuplicate = true; break;
      }
    }
    if (!isDuplicate) { unique.push(bullet); seen.add(normalized); }
  }

  return [...unique, ...closing];
}
...
async function softCheck(file) {
  if (isHeicFile(file)) {
    return {
      isHeic: true,
      heicMessage: "iPhone HEIC photos aren't supported yet — but the fix is quick!\n\n1. On your iPhone, go to Settings → Camera → Formats\n2. Select \"Most Compatible\"\n3. Take a new photo — it will save as JPEG and work perfectly\n\nAlternately, take a screenshot of the photo and upload that instead."
    };
  }
  const warnings = [];
  ...
  return new Promise(resolve => {
    ...
    img.onload = () => {
      if (img.width < 150 || img.height < 150) warnings.push("Resolution is very low. Try getting closer or using a higher quality camera setting.");
      ...
      if(variance<120){
        if(brightness<35) warnings.push("Photo is too dark. Enable your camera flash and try again.");
        else if(brightness>230) warnings.push("Photo is too bright/washed out. Adjust your angle and try again.");
        else warnings.push("The photo doesn't show much detail — make sure the correct body area fills most of the frame.");
      }
      URL.revokeObjectURL(url);
      resolve({isHeic:false,warnings});
    };
...
async function generatePDF(dogInfo, results, weekLabel) {
  ...
  results.forEach(r=>{
    ...
    if(s2>0){
      ...
      doc.setTextColor(122,153,144); doc.setFont("helvetica","italic"); doc.text("Not analyzed",m+20,y+4);
    }
    y+=10;
  ...
  const bullets = deduplicateBullets(results);
  if(bullets.length){
    ...
    const closingIdx = bullets.findIndex(b => b.toLowerCase().includes("nobLfoods.com") || b.toLowerCase().includes("customerservice"));
    bullets.forEach((bullet,i)=>{
      if(y>H-m){doc.addPage();y=m;}
      const isClosing = i===closingIdx;
      if(isClosing&&i>0){ y+=3; doc.setDrawColor(114,170,185); doc.line(m,y,W-m,y); y+=4; }
      doc.setFont("helvetica",isClosing?"italic":"normal");
      doc.setTextColor(isClosing?114:74,isClosing?170:102,isClosing?185:96);
      const lines=doc.splitTextToSize("• "+bullet,W-m*2-6); doc.text(lines,m+4,y); y+=lines.length*5+2;
    });
    y+=4;
  }
...
function PhotoCapture({zones,onComplete}){
  ...
  const acceptFile=(file,b64)=>{
    setPhotos(p=>({...p,[zone.id]:b64}));
  ...
  const forceAccept=()=>{ if(pendingFile){
    if(pendingB64) acceptFile(pendingFile,pendingB64);
    else { const r=new FileReader(); r.onload=()=>acceptFile(pendingFile,r.result.split(",")[1]); r.readAsDataURL(pendingFile); }
  }};
        ...
        return(<div key={i} style={{marginBottom:"16px"}}>
          ...
          {(!r.score||r.score===0)?(
            <div style={{fontSize:"13px",color:C.textLight,fontStyle:"italic",paddingLeft:"4px",lineHeight:"1.5"}}>
              {r.photoNote||"Photo could not be analyzed. Please retake using the guide above and try again."}
            </div>
          ):(
    ...
    {bullets.length>0&&(
      <div style={S.cardForest}>
        ...
        {bullets.map((bullet,i)=>{
          const isClosing=i===closingIdx;
          return(<div key={i} style={{display:"flex",gap:"10px",alignItems:"flex-start",marginBottom:"8px",
            paddingTop:isClosing&&i>0?"10px":"0",
            borderTop:isClosing&&i>0?"1px solid rgba(255,255,255,0.15)":"none"}}>
            <div style={{fontSize:"14px",color:isClosing?C.skyLight:C.fog,flexShrink:0,marginTop:"1px"}}>•</div>
            <div style={{fontSize:"13px",color:isClosing?C.skyLight:C.fog,lineHeight:"1.6",fontStyle:isClosing?"italic":"normal"}}>{bullet}</div>
          </div>);
        })}
      </div>
    )}
  ...
  const handlePhotosComplete=useCallback(async(photos)=>{
    ...
    for(let i=0;i<PHOTO_ZONES.length;i++){
      ...
      try{
        ...
        results.push({zoneId:zone.id,score:0,observations:[],dietSignals:"",recommendations:"",photoNote:`Analysis failed for this area (${err.message||"connection error"}). Please check your internet and try again.`,trend:"baseline"});
      }
    }
  ...
  return(<div style={S.page}>
    ...
    <div style={S.body}>
      ...
      {step==="results"&&weekResults[viewingWeek]&&(
        <AnalysisResults
          results={weekResults[viewingWeek]} weekLabel={WEEKS[viewingWeek]} dogInfo={dogInfo}
